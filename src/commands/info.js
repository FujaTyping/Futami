...
class InfoCommand extends Command {
  ...
  async chatInputRun(interaction) {
    ...
    const Content = new EmbedBuilder()
      ...
      .setDescription(
        `เวลาที่ออนไลน์ : **${Uptime(os.uptime())}** || อยู่ใน **${client.guilds.cache.size}** เชิฟเวอร์`,
      )
      .addFields(
        {
          name: "🖥️ เครื่องเชิฟเวอร์",
          value: `ซีพียู (**${cpu.length}** คอร์) : **${cpu[0].model}**\nแรม : **${(os.totalmem() / 1024 ** 3).toFixed(2) + " กิกะไบต์"}**\`\nแพลตฟอร์ม : **${os.platform()}**\nไทพ์ : **${os.type()}**`,
          inline: true,
        },
        {
          name: "🖥️ รันไทม์",
          value: `ชีพียู : **${getCPUUsage()}**\nแรม : **${getRAMUsage()}**\nชาร์ด : **${client.options.shardCount}**`,
          inline: true,
        },
      )
      .setTimestamp();
