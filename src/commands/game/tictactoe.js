...
class TicTacToeCommand extends Command {
    ...
    registerApplicationCommands(registry) {
        registry.registerChatInputCommand((builder) =>
            builder.setName('tictactoe').setDescription('เล่นเกมเอกซ์-โอไหม ??')
    ...
    async chatInputRun(interaction) {
        if (interaction.user == interaction.options.getUser('user')) {
            const Content = new EmbedBuilder()
                ...
                .setDescription('เล่นเกมกับตัวเองไม่ได้นะ !! , ให้เพื่อนสักคนมาเล่นด้วยกันสิ 😀 หรือว่าจะมาคุยกับ ฟูตามิ แทนดี ?')
            ...
            const Game = new TicTacToe({
                ...
                winMessage: '🎉 ดีใจด้วย !! **{player}** ({emoji}) ชนะเกม เอกซ์-โอ ในรอบนี้',
