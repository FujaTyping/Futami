...
class AnimeCommand extends Subcommand {
  ...
  registerApplicationCommands(registry) {
    registry.registerChatInputCommand((builder) =>
      builder
        ...
        .addSubcommand((command) =>
          command.setName("random").setDescription("สุ่มอนิเมะสักเรื่องไหม ??"),
        )
        .addSubcommand((command) =>
          command
            .setName("character")
  ...
  async chatInputSearch(interaction) {
    ...
    axios
      .get(`https://api.jikan.moe/v4/anime?q=${Search}&limit=1`)
      .then(async (response) => {
        ...
        if (!Response) {
          const Content = new EmbedBuilder()
            ...
            .setTitle("👦🏻 ข้อมูลอนิเมะ")
            .setDescription(
              `ไม่พบข้อมูลของอนิเมะเรื่อง : **${Search}**\nลองเช็คดูว่าใส่ชื่ออนิเมะถูกมั้ย ??`,
            )
            .setTimestamp();
          ...
          const Content = new EmbedBuilder()
            ...
            .setTitle("👦🏻 ข้อมูลอนิเมะ")
            .setDescription(
              `${MainText.replaceAll("null", "-").replaceAll("undefined", "-")}`,
            )
            .setFooter({
              text: `${Response.producers[0].name} • ${Response.studios[0].name}`,
            })
            .setTimestamp();
          ...
          if (Response.trailer.url) {
            ...
            const Row = new ActionRowBuilder().addComponents(Info, MoreInfo);
          ...
          const Collector = msg.createMessageComponentCollector({
            filter: (buttonInteraction) =>
              buttonInteraction.customId === "synopsis" &&
      ...
      .catch((error) => {
        const Content = new EmbedBuilder()
          ...
          .setDescription(
            "**ลองเช็คดูว่าใส่ชื่ออนิเมะถูกไหม\n```\n" + error + "\n```",
          )
          .setTimestamp();
  ...
  async chatInputCharacter(interaction) {
    ...
    axios
      .get(`https://api.jikan.moe/v4/characters?q=${Search}&limit=1`)
      .then(async (response) => {
        const Response = response.data.data[0];
        ...
        if (!Response) {
          ...
          const Row = new ActionRowBuilder().addComponents(MoreInfo);
      ...
      .catch((error) => {
        const Content = new EmbedBuilder()
          ...
          .setDescription(
            "**ลองเช็คดูว่าใส่ชื่อตัวละครถูกไหม\n```\n" + error + "\n```",
          )
          .setTimestamp();
