...
class CalculatorCommand extends Command {
  ...
  registerApplicationCommands(registry) {
    registry.registerChatInputCommand((builder) =>
      builder
        .setName("calculator")
        .setDescription("ปวดห้วกับการคิดเลขไหม ??")
  ...
  async chatInputRun(interaction) {
    ...
    const Example = Choices.find(
      (choice) => choice.value === Operation,
    )?.example;
      ...
      .then((response) => {
        ...
        if (Response.result == "NaN") {
          const Content = new EmbedBuilder()
            ...
            .setTitle(`📳 เครื่องคิดเลข`)
            .setDescription(
              `ไม่สามารถแก้สมการ \`${Expression}\` โดยใช้ **${Type}** ได้\nตัวอย่างโจทย์ปัญหา ${Type} : ${Example}`,
            )
            .setTimestamp();
          ...
          const Content = new EmbedBuilder()
            ...
            .setTitle(`📳 เครื่องคิดเลข`)
            .setDescription(
              `ผลลัพธ์การแก้สมการโดยใช้ **${Type}** ด้วย \`${Expression}\` คือ\n` +
                "```\n" +
                Response.result +
                "\n```" +
                `\nตัวอย่างโจทย์ปัญหา ${Type} : ${Example}`,
