...
class UrlCommand extends Command {
    ...
    registerApplicationCommands(registry) {
        registry.registerChatInputCommand((builder) =>
            builder.setName('url').setDescription('มีปัญหากับลิ้งค์หรอ ??')
    ...
    async chatInputRun(interaction) {
        ...
        if (Action == 'shorten') {
            axios.get(`https://is.gd/create.php?format=simple&url=${Url}`)
                .then(response => {
                    const Response = response.data;

                    const Content = new EmbedBuilder()
                        .setColor(color)
                        .setAuthor({ name: 'ย่อลิ้งค์สำเร็จแล้ว !', iconURL: 'https://futami.siraphop.me/assets/icon/checked.png' })
                        .addFields(
                            { name: '**⛓️ ลิ้งค์ที่ย่อแล้ว**', value: `${Response}`, inline: true },
            ...
            axios.get(`https://unshorten.me/json/${Url}`)
                .then(response => {
                    const Response = response.data;

                    const Content = new EmbedBuilder()
                        .setColor(color)
                        .setAuthor({ name: 'ยกเลิกการย่อลิ้งค์สำเร็จแล้ว !', iconURL: 'https://futami.siraphop.me/assets/icon/checked.png' })
                        .addFields(
                            { name: '**⛓️‍💥 ลิ้งค์ต้อนฉบับ**', value: `${Response.resolved_url}`, inline: true },
                            { name: '**⛓️ ลิ้งค์ที่ย่อแล้ว**', value: `${Url}`, inline: true }
                        )
                        .setTimestamp()
