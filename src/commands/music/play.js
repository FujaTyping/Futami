...
class PlayCommand extends Command {
  ...
  async messageRun(message, args) {
    ...
    if (Song.includes("--playlist")) {
      ...
      const Playlist = DataPlaylist.Playlist.find(
        (PlaySong) => PlaySong.name === PlaylistName,
      );
      ...
      if (Playlist) {
        ...
        const Content = new EmbedBuilder()
          ...
          .setDescription(
            `เพลย์ลิส : **${Playlist.title}**\nเพิ่มโดย : \`${Playlist.request}\``,
          )
          .setTimestamp();

        return await msg.edit({
          content: "ดึงข้อมูลเพลงเสร็จล่ะ ✨",
          embeds: [Content],
        });
      } else {
        let PlaylistName = Song.replace("--playlist", "").trim();
        const Content = new EmbedBuilder()
          ...
          .setDescription(
            `ดูเหมือนว่าจะไม่มีเพลย์ลิส **${PlaylistName}** ในฐานข้อมูลของฟูตามินะ`,
          )
          .setTimestamp();
      ...
      return await msg.edit({
        content: "เจอเพลงล่ะ ✨" /*, embeds: [Content] */,
      });
    }
  }
}
